...
md`
An interactive graph convolutional network!  
  ...
  main.variable(observer("buttons_display")).define("buttons_display", ["html","mutable step","step","randomize_graph"], function(html,$0,step,randomize_graph)
{
  ...
  main.variable(observer("main_div")).define("main_div", ["html","style","step","W_slider","B_slider"], function(html,style,step,W_slider,B_slider)
{
  return html`
  ${style}
  <div id="main-div">
  ...
  main.variable(observer("simulation")).define("simulation", ["svg","d3","width"], function(svg,d3,width)
{
  ...
  main.variable(observer("eqn_display")).define("eqn_display", ["md","step","tex","main_node","formatted_main_node_new","formatted_neighbours_step_1","param_col","formatted_main_node","formatted_neighbours_step_2","B_curr","formatted_main_node_current_features","formatted_neighbours_step_3","format_feature","main_node_current_features","formatted_eqn_step_4","formatted_main_node_updated_features_before_f","formatted_main_node_updated_features"], function(md,step,tex,main_node,formatted_main_node_new,formatted_neighbours_step_1,param_col,formatted_main_node,formatted_neighbours_step_2,B_curr,formatted_main_node_current_features,formatted_neighbours_step_3,format_feature,main_node_current_features,formatted_eqn_step_4,formatted_main_node_updated_features_before_f,formatted_main_node_updated_features){return(
md`
  **Next Update (Iteration ${step + 1}):**  
  ...
  Note that the weights ${tex`${param_col} W^{(${step + 1})}`} and ${tex`${param_col} B^{(${step + 1})}`} are shared across all nodes!
`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`
...
function update_features(features, W_step, B_step){
  return features.map((node) => {
    ...
    if(neighbour_features.length > 0){
      neighbour_features_aggregrated = math.mean(neighbour_features, 0);
    } else {
  ...
  main.variable(observer("compute_features")).define("compute_features", ["W_update","B_update","original_features","copy","update_features","W_arr","B_arr","post_conv_func"], function(W_update,B_update,original_features,copy,update_features,W_arr,B_arr,post_conv_func){return(
function compute_features(step){
  ...
  for(var curr_step = 0; curr_step <= step; ++curr_step){  
    ...
    updated_features_before_f = update_features(features, W_arr[curr_step], B_arr[curr_step]);
...
"{" + neigh_col + "h_{" + neighbour.name.toString() + "}^{(" + step + ")}}").join(" + ")
  ...
  main.variable(observer("formatted_neighbours_step_1")).define("formatted_neighbours_step_1", ["main_node_neighbours","param_col","step","formatted_neighbours","main_node_num_neighbours"], function(main_node_neighbours,param_col,step,formatted_neighbours,main_node_num_neighbours){return(
main_node_neighbours.length > 0 ?
`{${param_col} W^{(${step + 1})}} \\times \\frac{${formatted_neighbours}}{${main_node_num_neighbours}} + ` : ``
)});
  main.variable(observer("formatted_neighbours_step_2")).define("formatted_neighbours_step_2", ["main_node_neighbours","param_col","W_curr","formatted_neighbours_values","main_node_num_neighbours"], function(main_node_neighbours,param_col,W_curr,formatted_neighbours_values,main_node_num_neighbours){return(
main_node_neighbours.length > 0 ?
`{${param_col} ${W_curr}} \\times \\frac{${formatted_neighbours_values}}{${main_node_num_neighbours}} + ` : ``
)});
