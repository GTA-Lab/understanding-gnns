...
md`
An interactive GraphSAGE model!  
  ...
  main.variable(observer("buttons_display")).define("buttons_display", ["html","mutable step","step","randomize_graph"], function(html,$0,step,randomize_graph)
{
  ...
  main.variable(observer("main_div")).define("main_div", ["html","style","step","W_slider","B_slider","U_s_slider","U_h_slider"], function(html,style,step,W_slider,B_slider,U_s_slider,U_h_slider)
{
  return html`
  ${style}
  <div id="main-div">
  ...
  main.variable(observer("simulation")).define("simulation", ["svg","d3","width"], function(svg,d3,width)
{
  ...
  main.variable(observer("eqn_display")).define("eqn_display", ["md","step","tex","main_node","formatted_main_node_new","formatted_neighbours_step_1","param_col","formatted_main_node","formatted_neighbours_step_2","B_curr","formatted_main_node_current_features","formatted_neighbours_step_3","format_feature","main_node_current_features","formatted_eqn_step_4","formatted_main_node_updated_features_before_f","formatted_main_node_updated_features","formatted_neighbour_names","formatted_rnn_computation","formatted_rnn_last_state_value","formatted_rnn_last_state","formatted_rnn_computation_concise","main_col"], function(md,step,tex,main_node,formatted_main_node_new,formatted_neighbours_step_1,param_col,formatted_main_node,formatted_neighbours_step_2,B_curr,formatted_main_node_current_features,formatted_neighbours_step_3,format_feature,main_node_current_features,formatted_eqn_step_4,formatted_main_node_updated_features_before_f,formatted_main_node_updated_features,formatted_neighbour_names,formatted_rnn_computation,formatted_rnn_last_state_value,formatted_rnn_last_state,formatted_rnn_computation_concise,main_col){return(
md`
    ...
    ${
      ...
      ${formatted_main_node_new}
                &= {f} \left(
                    ...
                    {${param_col} B^{(${step + 1})}} \times ${formatted_main_node} \right) \\ 
                &= f \left(
                    ${formatted_neighbours_step_2}
                    {${param_col} ${B_curr}} \times ${formatted_main_node_current_features} \right) \\
                &= f \left(
                    ${formatted_neighbours_step_3}
                    ${format_feature(B_curr * main_node_current_features)} \right) \\
  ...
  Considering the sequence of neighbours as ${tex`[${formatted_neighbour_names}]`}, the ${tex`\text{RNN}`} computes:
  ${
    tex.block`
      \begin{aligned}
        \qquad s_0 &= 0. \\
        ${formatted_rnn_computation}
      \end{aligned}
    `
  }
  The returned aggregated value ${tex`${formatted_rnn_last_state_value}`} is just the last state ${tex`${formatted_rnn_last_state}`} of the ${tex`\text{RNN}`}.

  Concisely, the initial ${tex`\text{RNN}`} state is ${tex`s_0 = 0`}, and 
  the ${tex`\text{RNN}`} state update equation is:
  ${
    tex.block`
    \begin{aligned}
      ${formatted_rnn_computation_concise}
    \end{aligned}
    `
  }
  where ${tex`u_t`} is the ${tex`t`}-th neighbour of ${tex`${main_col} ${main_node.name}`} in the considered sequence.

  In practice, we would consider different permutations of the neighbour sequence, and train the
  ${tex`\text{RNN}`} to learn the same final aggregated value.
  This tries to make the ${tex`\text{RNN}`} node-order invariant.


  Note that the weights ${tex`${param_col} W^{(${step + 1})}`}, ${tex`${param_col} B^{(${step + 1})}`}, ${tex`${param_col} U_s^{(${step + 1})}`} and ${tex`${param_col} U_h^{(${step + 1})}`} are shared across all nodes!
`
)});
  main.variable(observer("formatted_rnn_computation_concise")).define("formatted_rnn_computation_concise", ["param_col","step","U_s_curr","U_h_curr"], function(param_col,step,U_s_curr,U_h_curr){return(
`s_{t} &= \\text{ReLU}\\left({${param_col} U_s} s_{t - 1} + {${param_col} U_h} h_{u_t}^{(${step})}\\right) \\\\ &= \\text{ReLU}\\left({${param_col} ${U_s_curr}} \ s_{t - 1} + {${param_col} ${U_h_curr}} \ h_{u_t}^{(${step})}\\right).`
)});
  main.variable(observer("formatted_rnn_computation")).define("formatted_rnn_computation", ["main_node_neighbours","param_col","neigh_col","step","U_s_curr","format_feature","main_node_rnn_states","U_h_curr","main_node_neighbours_values","main_node_rnn_states_pre_nonlinearity"], function(main_node_neighbours,param_col,neigh_col,step,U_s_curr,format_feature,main_node_rnn_states,U_h_curr,main_node_neighbours_values,main_node_rnn_states_pre_nonlinearity){return(
main_node_neighbours.map(
  (neighbour, t) => 
  `s_{` + (t + 1).toString() + `}`+
  `&= \\text{ReLU}\\left({${param_col} U_s} s_{${t}} + {${param_col} U_h} {${neigh_col} h_{${neighbour.name}}^{(${step})}}\\right)` +
  ` = \\text{ReLU}\\left(
      {${param_col} ${U_s_curr}}
      \\times ${format_feature(main_node_rnn_states[t])} +    
      {${param_col} ${U_h_curr}}    
      \\times {${neigh_col} ${format_feature(main_node_neighbours_values[t])}} \\right)` +
  ` = \\text{ReLU}\\left(${format_feature(main_node_rnn_states_pre_nonlinearity[t + 1])}\\right)` +
  ` = ${format_feature(main_node_rnn_states[t + 1])}.`
).join(` \\\\ `)
)});
  main.variable(observer("formatted_rnn_last_state_value")).define("formatted_rnn_last_state_value", ["neigh_col","format_feature","main_node_rnn_states","main_node_neighbours"], function(neigh_col,format_feature,main_node_rnn_states,main_node_neighbours){return(
`${neigh_col} ${format_feature(main_node_rnn_states[main_node_neighbours.length])}`
)});
  main.variable(observer("formatted_rnn_last_state")).define("formatted_rnn_last_state", ["main_node_neighbours"], function(main_node_neighbours){return(
`s_{${main_node_neighbours.length}}`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`
# Appendix
...
ReLU
  ...
  main.variable(observer("post_rnn_func")).define("post_rnn_func", ["ReLU"], function(ReLU){return(
ReLU
)});
...
Array(100).fill(1)
  ...
  main.variable(observer("U_s_curr")).define("U_s_curr", ["Generators","U_s_slider"], function(Generators,U_s_slider){return(
Generators.input(U_s_slider)
)});
  main.define("initial U_s_arr", function(){return(
Array(100).fill(1)
)});
  main.variable(observer("mutable U_s_arr")).define("mutable U_s_arr", ["Mutable", "initial U_s_arr"], (M, _) => new M(_));
  main.variable(observer("U_s_arr")).define("U_s_arr", ["mutable U_s_arr"], _ => _.generator);
  main.variable(observer("U_h_curr")).define("U_h_curr", ["Generators","U_h_slider"], function(Generators,U_h_slider){return(
Generators.input(U_h_slider)
)});
  main.define("initial U_h_arr", function(){return(
Array(100).fill(1)
)});
  main.variable(observer("mutable U_h_arr")).define("mutable U_h_arr", ["Mutable", "initial U_h_arr"], (M, _) => new M(_));
  main.variable(observer("U_h_arr")).define("U_h_arr", ["mutable U_h_arr"], _ => _.generator);
  main.variable(observer("B_curr")).define("B_curr", ["Generators","B_slider"], function(Generators,B_slider){return(
Generators.input(B_slider)
)});
  main.define("initial B_arr", function(){return(
Array(100).fill(1)
)});
  main.variable(observer("mutable B_arr")).define("mutable B_arr", ["Mutable", "initial B_arr"], (M, _) => new M(_));
  main.variable(observer("B_arr")).define("B_arr", ["mutable B_arr"], _ => _.generator);
  main.variable(observer("W_update")).define("W_update", ["mutable W_arr","step","W_curr"], function($0,step,W_curr)
{
  ...
  main.variable(observer("U_s_update")).define("U_s_update", ["mutable U_s_arr","step","U_s_curr"], function($0,step,U_s_curr)
{
  $0.value[step] = U_s_curr;
  $0.value = $0.value;
}
);
  main.variable(observer("U_h_update")).define("U_h_update", ["mutable U_h_arr","step","U_h_curr"], function($0,step,U_h_curr)
{
  $0.value[step] = U_h_curr;
  $0.value = $0.value;
}
);
  main.variable(observer("B_update")).define("B_update", ["mutable B_arr","step","B_curr"], function($0,step,B_curr)
{
  $0.value[step] = B_curr;
  $0.value = $0.value;
}
...
slider({
  ...
  main.variable(observer("B_slider")).define("B_slider", ["slider","tex","step"], function(slider,tex,step){return(
slider({
  min: -10,
  max: 10, 
  step: 0.1,
  ...
  main.variable(observer("U_s_slider")).define("U_s_slider", ["slider","tex","step"], function(slider,tex,step){return(
slider({
  min: -10,
  max: 10, 
  step: 0.1,
  value: this ? this.input.value : 1,
  title: tex`U_s^{(${step + 1})}`,
})
)});
  main.variable(observer("U_h_slider")).define("U_h_slider", ["slider","tex","step"], function(slider,tex,step){return(
slider({
  min: -10,
  max: 10, 
  step: 0.1,
  value: this ? this.input.value : 1,
  title: tex`U_h^{(${step + 1})}`,
})
)});
  main.variable(observer()).define(["md"], function(md){return(
md`
...
`
  ...
  main.variable(observer("RNN_aggregate")).define("RNN_aggregate", ["post_rnn_func","ReLU"], function(post_rnn_func,ReLU){return(
function RNN_aggregate(neighbour_features, U_s_step, U_h_step){
  function RNN_state_sequence(state_sequence_so_far, new_neighbour_feature){
    var prev_state = state_sequence_so_far[state_sequence_so_far.length - 1];
    var new_state = post_rnn_func(U_s_step * prev_state + U_h_step * new_neighbour_feature);
    return state_sequence_so_far.concat([new_state]);
  }
  
  function RNN_state_sequence_pre_nonlinearity(state_sequence_so_far, new_neighbour_feature){
    var prev_state = ReLU(state_sequence_so_far[state_sequence_so_far.length - 1]);
    var new_state_pre_nonlinearity = U_s_step * prev_state + U_h_step * new_neighbour_feature;
    return state_sequence_so_far.concat([new_state_pre_nonlinearity]);
  }
  
  var rnn_states = neighbour_features.reduce(RNN_state_sequence, [0]);
  var rnn_states_pre_nonlinearity = neighbour_features.reduce(RNN_state_sequence_pre_nonlinearity, [0]);
  var aggregated_neighbour_features = rnn_states[rnn_states.length - 1];
  return {
    rnn_states,
    rnn_states_pre_nonlinearity,
    aggregated_neighbour_features
  };
}
)});
  main.variable(observer("compute_aggregated_neighbour_features")).define("compute_aggregated_neighbour_features", ["get_neighbours_features","RNN_aggregate"], function(get_neighbours_features,RNN_aggregate){return(
function compute_aggregated_neighbour_features(features, U_s_step, U_h_step){
  return features.map((node) => {
    const neighbour_features = get_neighbours_features(features, node.id);
    const { 
      aggregated_neighbour_features,
      rnn_states,
      rnn_states_pre_nonlinearity,
     } = RNN_aggregate(neighbour_features, U_s_step, U_h_step);
    
    return {
      "id": node.id,
      "aggregated_neighbour_features": aggregated_neighbour_features,
      "rnn_states": rnn_states,
      "rnn_states_pre_nonlinearity": rnn_states_pre_nonlinearity,
    }
  });
}
)});
  main.variable(observer("update_features")).define("update_features", ["get_neighbours_features","RNN_aggregate","math"], function(get_neighbours_features,RNN_aggregate,math){return(
function update_features(features, W_step, B_step, U_s_step, U_h_step){
  return features.map((node) => {
    const neighbour_features = get_neighbours_features(features, node.id);
    
    var neighbour_features_aggregrated;
    if(neighbour_features.length > 0){
      neighbour_features_aggregrated = RNN_aggregate(neighbour_features,
                                                     U_s_step, U_h_step).aggregated_neighbour_features;
    } else {
      neighbour_features_aggregrated = 0;
    }

    var node_updated_features_before_f = math.add(math.multiply(W_step, neighbour_features_aggregrated),
                                                  math.multiply(B_step, node.features));
  ...
  main.variable(observer("compute_features")).define("compute_features", ["W_update","B_update","U_s_update","U_h_update","original_features","copy","compute_aggregated_neighbour_features","U_s_arr","U_h_arr","update_features","W_arr","B_arr","post_conv_func"], function(W_update,B_update,U_s_update,U_h_update,original_features,copy,compute_aggregated_neighbour_features,U_s_arr,U_h_arr,update_features,W_arr,B_arr,post_conv_func){return(
function compute_features(step){
  W_update; B_update; U_s_update; U_h_update;

  var current_features, aggregated_neighbour_features, updated_features_before_f, updated_features;
 
  var features = original_features;
  for(var curr_step = 0; curr_step <= step; ++curr_step){  
    current_features = copy(features);
    aggregated_neighbour_features = compute_aggregated_neighbour_features(features, 
                                                                          U_s_arr[curr_step],
                                                                          U_h_arr[curr_step]);
    updated_features_before_f = update_features(features,
                                                W_arr[curr_step], B_arr[curr_step],
                                                U_s_arr[curr_step], U_h_arr[curr_step]);
...
all_features.updated_features
  ...
  main.variable(observer("aggregated_neighbour_features")).define("aggregated_neighbour_features", ["all_features"], function(all_features){return(
all_features.aggregated_neighbour_features
)});
  main.variable(observer()).define(["md"], function(md){return(
md`
### Graph Utils
...
function get_neighbours(node_id){
  ...
  main.variable(observer("get_neighbours_features")).define("get_neighbours_features", ["get_neighbours"], function(get_neighbours){return(
function get_neighbours_features(features, node_id){
  const neighbour_ids = get_neighbours(node_id).map((o_node) => (o_node.id));
  const neighbour_features = neighbour_ids.map(
...
main_node_updated_features_before_f.map(ReLU)
  ...
  main.variable(observer("main_node_neighbours_values")).define("main_node_neighbours_values", ["main_node_neighbours","current_features"], function(main_node_neighbours,current_features){return(
main_node_neighbours.map(
  (neighbour) => current_features.find((node) => (node.id == neighbour.id)).features
)
)});
  main.variable(observer("main_node_neighbours_values_agg")).define("main_node_neighbours_values_agg", ["aggregated_neighbour_features","main_node_id"], function(aggregated_neighbour_features,main_node_id){return(
aggregated_neighbour_features.find(
  (node) => (node.id == main_node_id)
).aggregated_neighbour_features
)});
  main.variable(observer("main_node_rnn_states")).define("main_node_rnn_states", ["aggregated_neighbour_features","main_node_id"], function(aggregated_neighbour_features,main_node_id){return(
aggregated_neighbour_features.find(
  (node) => (node.id == main_node_id)
).rnn_states
)});
  main.variable(observer("main_node_rnn_states_pre_nonlinearity")).define("main_node_rnn_states_pre_nonlinearity", ["aggregated_neighbour_features","main_node_id"], function(aggregated_neighbour_features,main_node_id){return(
aggregated_neighbour_features.find(
  (node) => (node.id == main_node_id)
).rnn_states_pre_nonlinearity
)});
...
format_feature(main_node_updated_features_before_f)
  ...
  main.variable(observer("formatted_neighbour_names")).define("formatted_neighbour_names", ["main_node_neighbours","neigh_col"], function(main_node_neighbours,neigh_col){return(
main_node_neighbours.map((neighbour) => 
"{" + neigh_col + neighbour.name.toString() + "}").join(" , ")
)});
...
"{" + neigh_col + "h_{" + neighbour.name.toString() + "}^{(" + step + ")}}").join(" , ")
  ...
  main.variable(observer("formatted_neighbours_step_1")).define("formatted_neighbours_step_1", ["main_node_neighbours","param_col","step","formatted_neighbours"], function(main_node_neighbours,param_col,step,formatted_neighbours){return(
main_node_neighbours.length > 0 ?
`{${param_col} W^{(${step + 1})}} \\times \\text{RNN}\\left[${formatted_neighbours}\\right] + ` : ``
)});
  main.variable(observer("formatted_neighbours_step_2")).define("formatted_neighbours_step_2", ["main_node_neighbours","param_col","W_curr","formatted_neighbours_values_agg"], function(main_node_neighbours,param_col,W_curr,formatted_neighbours_values_agg){return(
main_node_neighbours.length > 0 ?
`{${param_col} ${W_curr}} \\times ${formatted_neighbours_values_agg} + ` : ``
)});
  main.variable(observer("formatted_neighbours_step_3")).define("formatted_neighbours_step_3", ["main_node_neighbours","format_feature","W_curr","main_node_neighbours_values_agg"], function(main_node_neighbours,format_feature,W_curr,main_node_neighbours_values_agg){return(
main_node_neighbours.length > 0 ?
`${format_feature(W_curr * main_node_neighbours_values_agg)} + `: ``
)});
...
`&= f \\left(${formatted_main_node_updated_features_before_f} \\right) \\\\` : ``
  ...
  main.variable(observer("formatted_neighbours_values_agg")).define("formatted_neighbours_values_agg", ["neigh_col","format_feature","main_node_neighbours_values_agg"], function(neigh_col,format_feature,main_node_neighbours_values_agg){return(
"{" + neigh_col +
format_feature(main_node_neighbours_values_agg) +
"}"
)});
